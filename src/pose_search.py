        ...
        if apply_tilting_scheme:
            ...
            if n_in_planes > 1:  # first step
                ...
                for chunk_idx in range(n_in_planes * n_tilts):
                    y = model.eval_on_slice(x[:, :, chunk_idx], z=z, class_idx=class_idx)
                    y = y.float()  # [b, nq, npts]
                    if z is None:
            ...
            # z: [b, z_dim], [b * nq, z_dim]
            y_hat = model.eval_on_slice(x, z=z, class_idx=class_idx)
...
def opt_theta_trans_multi_class(
        model,
        images,
        lattice,
        ps_params,
    ...
    output: [batch_size (n_tilts,) (n_classes,) 3, 3], [batch_size, (n_tilts,) (n_classes) 2], ?
    """
    if n_classes == 1:
        return opt_theta_trans(model, images, lattice, ps_params, z=z, ctf_i=ctf_i, gt_trans=gt_trans,
                               trans_search_factor=trans_search_factor)
    else:
        best_rot_list = []
        best_trans_list = []
        for k in range(n_classes):
            best_rot, best_trans, _ = opt_theta_trans(
                model,
                images,
                lattice,
                ps_params,
                z=z[:, k] if z is not None else None,
                ctf_i=ctf_i,
                gt_trans=gt_trans,
                trans_search_factor=trans_search_factor,
                class_idx=k
            )
            best_rot_list.append(best_rot[..., None, :, :])
            best_trans_list.append(best_trans[..., None, :])
        best_rot = torch.cat(best_rot_list, dim=-3)
        best_trans = torch.cat(best_trans_list, dim=-2)
        return best_rot, best_trans, None
...
def opt_theta_trans(
    ...
    z = to_tensor(z) if z is not None else None
