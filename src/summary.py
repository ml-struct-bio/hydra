...
def make_class_summary(writer, p_classes, steps, labels, prefix='', pca=None, logvar=None,
                       palette_type=None, idx_best_class=None, n_classes=1):
    if n_classes > 1:
        n_per_class = []
        for k in range(n_classes):
            n_per_class.append((idx_best_class == k).sum())
        n_per_class = np.array(n_per_class)
        ratio_per_class = n_per_class / n_per_class.sum()

        fig = plt.figure(dpi=96, figsize=(6, 4))
        plt.bar(range(len(ratio_per_class)), ratio_per_class, width=0.5, facecolor='cornflowerblue')
        plt.xticks(range(len(ratio_per_class)))
        plt.ylim(0, 1)
        plt.grid(True)
        writer.add_figure(prefix + 'Ratio per Class', fig, global_step=steps)

        # Entropy distribution
        entropy_classes = entropy(p_classes, axis=-1)
        fig = plt.figure(dpi=96, figsize=(6, 4))
        plt.hist(entropy_classes, bins=50)
        plt.xlim(0, np.log(n_classes))
        plt.grid(True)
        plt.xlabel('Class Entropy (nats)')
        plt.ylabel('# occurrences')
        writer.add_figure(prefix + 'Entropy Histogram', fig, global_step=steps)
        writer.add_histogram(prefix + 'Entropy Histogram 2', entropy_classes, global_step=steps)

        if n_classes == 3:
            x, y = p_to_xy_triangle(p_classes)

            fig = plt.figure(figsize=(5, 5))
            plt.hist2d(x, y, cmap='Blues', bins=25)
            plt.xlim([0, 1])
            plt.ylim([0, np.sqrt(3) / 2])
            plt.axis('off')
            plt.plot([0.5], [np.sqrt(3) * 0.5], 'ro', label='0', markersize=15)
            plt.plot([0], [0], 'bo', label='1', markersize=15)
            plt.plot([1], [0], 'go', label='2', markersize=15)
            plt.legend(loc='upper right')
            writer.add_figure(prefix + 'Posterior over Classes', fig, global_step=steps)

def make_conf_summary(writer, conf, steps, labels, prefix='', pca=None, logvar=None,
                      palette_type=None, idx_best_class=None, n_classes=1, p_classes=None):
    if conf.shape[1] > 1:
        if idx_best_class is None:
            if pca is None:
            ...
            if labels is not None:
                # colored PCA
                centers = []
                n_ref_classes = int(np.max(labels) - np.min(labels)) + 1
                for i in range(n_ref_classes):
                    z_sub = conf[labels == i]
                    centers.append(np.mean(z_sub, axis=0))
                centers = np.array(centers)
                centers2, centers_i = analysis.get_nearest_point(conf, centers)
                fig, _ = analysis.plot_by_cluster(pc[:, 0], pc[:, 1], n_ref_classes, labels, centers_ind=centers_i,
                                                  annotate=True, dpi=96, figsize=(5, 5), s=5)
                writer.add_figure(prefix + "Conformation - Colored PCA", fig, global_step=steps)
        else:
            for k in range(n_classes):
                if (idx_best_class == k).sum() >= conf.shape[1]:
                    pred_class_mask = idx_best_class == k
                    filtered_conf = conf[pred_class_mask]
                    pc, pca = analysis.run_pca(filtered_conf)

                    fig = plt.figure(dpi=96, figsize=(5, 5))
                    plt.hexbin(pc[:, 0], pc[:, 1], cmap='Blues', gridsize=35)
                    plt.xlabel('PC1')
                    plt.ylabel('PC2')
                    writer.add_figure(prefix + "Conformation - PCA - Class " + str(k), fig, global_step=steps)
                    if labels is not None:
                        filtered_labels = labels[pred_class_mask]
                        # colored PCA
                        centers = []
                        n_ref_classes = int(np.max(labels) - np.min(labels)) + 1
                        for i in range(n_ref_classes):
                            z_sub = filtered_conf[filtered_labels == i]
                            centers.append(np.mean(z_sub, axis=0))
                        centers = np.array(centers)
                        centers2, centers_i = analysis.get_nearest_point(filtered_conf, centers)
                        fig, _ = analysis.plot_by_cluster(pc[:, 0], pc[:, 1], n_ref_classes, filtered_labels, centers_ind=centers_i,
                                                          annotate=True, dpi=96, figsize=(5, 5), s=5, legend=True)
                        writer.add_figure(prefix + "Conformation - Colored PCA - Class " + str(k), fig, global_step=steps)


    if conf.shape[1] == 1:
        if labels is not None and n_classes == 1:
            fig = plt.figure(dpi=96, figsize=(6, 4))
            ...
            if n_classes == 1:
                fig = plt.figure(dpi=96, figsize=(6, 4))
                plt.hist(conf[:, 0].flatten(), bins=50, histtype='step')
                writer.add_figure(prefix + "Conformation - Histogram", fig, global_step=steps)
            else:
                for k in range(n_classes):
                    if (idx_best_class == k).sum() > 0:
                        fig = plt.figure(dpi=96, figsize=(6, 4))
                        plt.hist(conf[idx_best_class == k, 0].flatten(), bins=50, histtype='step')
                        writer.add_figure(prefix + "Conformation - Histogram " + str(k), fig, global_step=steps)

    if logvar is not None and n_classes == 1:
